...
revealGroups.forEach(group=>{
  ...
  items.forEach((item,index)=>item.style.setProperty('--reveal-index',Math.min(index,8)));
});
const revealTargets=[...document.querySelectorAll('[data-reveal]'),...revealGroups];
const revealObserver=(!prefersReducedMotion&&'IntersectionObserver'in window)?new IntersectionObserver(entries=>{
...
const applyProjectPreview=(media,project)=>{
  ...
  const img=media.querySelector('.project-preview');
  if(!img)return;
  const previewUrl=project.open_graph_image_url||(project.full_name?`https://opengraph.githubassets.com/1/${project.full_name}`:'');
...
const renderProjects=projects=>{
  ...
  if(!projects.length){const empty=document.createElement('p');empty.textContent='No projects to show right now. Check back soon!';projectsGrid.append(empty);return;}
...
const loadProjects=async()=>{
  ...
  try{
    ...
    const filtered=data.filter(repo=>!repo.fork&&!repo.archived).sort(projectSorter).slice(0,12);
